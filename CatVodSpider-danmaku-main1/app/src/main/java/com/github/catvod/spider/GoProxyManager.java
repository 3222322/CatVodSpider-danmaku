...
public class GoProxyManager {
    ...
    static void startGoProxyOnceSync(Context context, int sessionId) {
        synchronized (isProxyRunning) {
            ...
            if (hasBinaryAssets) {
                startGoProxyBinarySync(context, sessionId);
            } else {
                log("[启动] Go代理所有方式均不可用");
                isProxyRunning.set(false);
            }
    ...
    private static void startGoProxyBinarySync(Context context, int sessionId) {
        try {
            if (!isStartSessionActive(sessionId)) {
                log("[二进制] 启动任务已取消，忽略本次Go代理启动");
                isProxyRunning.set(false);
                return;
            }
            ...
            for (int port : buildCandidatePorts()) {
                ...
                if (tcpOk) {
                    // TCP通了，重试HTTP检查（Go可能需要更长时间初始化HTTP端点）
                    HealthProbeResult probe = null;
                    for (int retry = 0; retry < 3; retry++) {
                        ...
                        probe = silentHealthCheck(port, 2000);
                        if (probe.healthy) break;
                        if (retry < 2) {
                            try { Thread.sleep(500); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        }
                    }
                    if (probe != null && probe.healthy) {
                        if (!isStartSessionActive(sessionId)) {
                            log("[二进制] 确认HTTP服务前检测到取消，忽略结果");
                            isProxyRunning.set(false);
                            return;
                        }
                        if (isGoHealthResponse(probe.body, port)) {
                            currentBackendPort = port;
                            foundHealthy = true;
                            break;
                        }
                        if (isBinaryStartupConfirmed(logFile, port)) {
                            currentBackendPort = port;
                            foundHealthy = true;
                            log("[探测] 端口 " + port + " 已确认HTTP服务运行(非标准响应)，且启动日志匹配，视为Go代理");
                            break;
                        }
                        log("[探测] 端口 " + port + " HTTP可达但启动日志不匹配，继续扫描");
                    }
                    log("[探测] 端口 " + port + " TCP可连但HTTP无响应，继续扫描");
                    continue;
                }
            }
            if (foundHealthy) {
                if (!isStartSessionActive(sessionId)) {
                    log("[二进制] 启动成功前检测到取消，忽略结果");
                    isProxyRunning.set(false);
                    return;
                }
    ...
    private static boolean isBinaryStartupConfirmed(java.io.File logFile, int port) {
        if (logFile == null || !logFile.exists()) return false;
        String successMark = "服务器启动在 :" + port;
        String bindErrorMark = "listen tcp :" + port + ": bind: address already in use";
        boolean started = false;
        try (java.io.BufferedReader br = new java.io.BufferedReader(new java.io.FileReader(logFile))) {
            String line;
            int lineCount = 0;
            while ((line = br.readLine()) != null && lineCount < 20) {
                if (line.contains(bindErrorMark)) return false;
                if (line.contains(successMark)) started = true;
                lineCount++;
            }
        } catch (Exception ignored) {
            return false;
