...
const pageCache = new LRUCache(500, 5 * 60 * 1000);
const tmdbSearchCache = new LRUCache(1000, 7 * 24 * 60 * 60 * 1000);
...
const fmtNextAirDate = (dateStr) => {
    const fmt = (d) => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit' }).format(d);
    const today = fmt(new Date());
    if (dateStr < today) return null;
    if (dateStr === today) return '今日更新';
    const tomorrow = fmt(new Date(Date.now() + 86400000));
    if (dateStr === tomorrow) return '明日更新';
    return `${parseInt(dateStr.slice(5, 7), 10)}月${parseInt(dateStr.slice(8), 10)}更新`;
};

const getLastAirToday = (detail, altDate) => {
    const today = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date());
    if (altDate && altDate === today) return '今日已更';
    const lastDate = detail?.last_episode_to_air?.air_date;
    if (lastDate === today) return '今日已更';
    return null;
};
...
const toVodFromTmdb = (item) => {
    ...
    return {
        ...
        vod_year: nextAirDate ? (fmtNextAirDate(nextAirDate) || getLastAirToday(item) || yearOf(item)) : (getLastAirToday(item) || yearOf(item)),
...
const getTVmazeEpisodeText = async (name, year) => {
    ...
    const cacheKey = `ep:${name}_${year || 'no_year'}`;
    const cached = tvmazeCache.get(cacheKey);
    if (cached !== undefined) return cached;
    ...
    try {
        ...
        let maxDate = '';
        const shDate = (s) => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date(s));
        aired.forEach(ep => { const d = shDate(ep.airstamp); if (d > maxDate) maxDate = d; });
        let nextDate = '';
        for (const ep of episodes) {
            if (ep.airstamp && new Date(ep.airstamp) > now) {
                nextDate = shDate(ep.airstamp);
                break;
            }
        }
        ...
        const result = (() => {
            ...
            const text = show.status === 'Ended' ? `(全${aired.length}集)` : `(更至${aired.length}集)`;
            return { text, lastDate: maxDate, nextDate };
        })();
        tvmazeCache.set(cacheKey, result);
        return result;
    } catch (e) {
...
const batchGetTVmazeEpisodeTexts = async (items) => {
    ...
    const promises = items.map(async (item) => {
        const data = await getTVmazeEpisodeText(item.vod_name, item.vod_year);
        if (data) results.set(item.vod_id, data);
    });
    ...
    try {
        ...
        const items = data.items.map(item => {
            ...
            return {
                ...
                if (id === 'tv' || id === 'show') {
                    try {
                        const tvmazeMap = await batchGetTVmazeEpisodeTexts(items);
                        const tvHandled = new Set();
                        for (const item of items) {
                            item._origRemarks = item.vod_remarks;
                            const tvmaze = tvmazeMap.get(item.vod_id);
                            if (tvmaze) {
                                item.vod_remarks = `${item.vod_remarks}${tvmaze.text}`;
                                item._tvmazeDate = tvmaze.lastDate;
                                item._tvmazeNext = tvmaze.nextDate;
                    ...
                    if (detail) {
                        ...
                        const nextAirFmt = nextAirDate && fmtNextAirDate(nextAirDate);
                        if (nextAirFmt) { item.vod_year = nextAirFmt; }
                        else { const t = getLastAirToday(detail); if (t) item.vod_year = t; }
                    }
                });
            }
        }
        const tvHandledArr = items.filter(it => tvHandled.has(it.vod_id));
                    ...
                    if (mappings.length > 0) {
                        ...
                        for (let i = 0; i < mappings.length; i += batchSize) {
                            ...
                            details.forEach((d, idx) => {
                                if (d.status === 'fulfilled' && d.value?.id) {
                                    const item = tvHandledArr.find(it => it.vod_id === batch[idx].vod_id);
                                    if (item) {
                                        const detail = d.value;
                                        const tmdbText = getTmdbEpisodeText(detail);
                                        const tmdbDate = detail.last_episode_to_air?.air_date || '';
                                        const tmdbNewer = tmdbText && tmdbDate >= (item._tvmazeDate || '');
                                        if (tmdbNewer) {
                                            item.vod_remarks = `${item._origRemarks}${tmdbText}`;
                                        }
                                        const tmdbNext = detail.next_episode_to_air?.air_date;
                                        const tmdbNextFmt = tmdbNext && fmtNextAirDate(tmdbNext);
                                        if (tmdbNextFmt) { item.vod_year = tmdbNextFmt; }
                                        else if (item._tvmazeNext) { const f = fmtNextAirDate(item._tvmazeNext); if (f) item.vod_year = f; }
                                        else { const t = getLastAirToday(detail, item._tvmazeDate); if (t) item.vod_year = t; }
                                    }
                                }
                            });
                            if (i + batchSize < mappings.length) await new Promise(r => setTimeout(r, 300));
                ...
                log.warn(`TMDB 集数获取失败: ${e.message}`);
            }
        }
...
const fetchHomeList = async () => {
    try {
        ...
        if (tvData?.items) {
            ...
            try {
                const tvmazeMap = await batchGetTVmazeEpisodeTexts(tvItems);
                const tvHandled = new Set();
...
const loadTmdbAnimeRanking = async ({ page, sort_by, year, region }) => {
    ...
    const fetchPromise = (async () => {
        try {
            ...
            const tvmazeMap = await batchGetTVmazeEpisodeTexts(results.map(r => ({ vod_id: String(r.id), vod_name: r.name || r.original_name || '', vod_year: yearOf(r) })));
            if (tvmazeMap.size > 0) {
                for (const vod of list) {
                    const tv = tvmazeMap.get(vod.vod_id);
                    if (!tv) continue;
                    const detail = results.find(r => String(r.id) === vod.vod_id);
                    const tmdbDate = detail?.last_episode_to_air?.air_date || '';
                    if (tv.lastDate > tmdbDate) {
                        if (tv.text) vod.vod_remarks = `${vod.vod_remarks}`.replace(/\([^)]+\)/g, '') + tv.text;
                    }
                    if (/^\d{4}$/.test(vod.vod_year)) {
                        const tmdbNext = detail?.next_episode_to_air?.air_date;
                        const tmdbNextFmt = tmdbNext && fmtNextAirDate(tmdbNext);
                        if (tmdbNextFmt) { vod.vod_year = tmdbNextFmt; }
                        else if (tv.nextDate) { const f = fmtNextAirDate(tv.nextDate); if (f) vod.vod_year = f; }
                        else { const t = getLastAirToday(detail, tv.lastDate); if (t) vod.vod_year = t; }
                    }
                }
            }
...
const handleT4Request = async (req) => {
    ...
    if (t) {
        ...
        if (t === 'anime_tmdb') return loadTmdbAnimeRanking({ page, sort_by: filters.tmdb_sort || 'hot_following', year: filters.tmdb_year || 'all', region: filters.tmdb_region || 'cn' });
...
module.exports = async (server, opt) => {
    ...
    opt.sites.push({
        key: "1", name: "豆瓣推荐", type: 4, api: apiPath,
