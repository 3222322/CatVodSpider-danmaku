...
public class DanmakuScanner {
    ...
    // 弹幕源URL（本插件推送的格式：.../api/v2/comment/{epId}?format=xml）
    private static final Pattern DANMAKU_URL_PATTERN = Pattern.compile(
            "(?i)[^ ]*?/api/v2/comment/([0-9]+)\\b"
    );
    // 通用格式：.../comment/{epId}...
    private static final Pattern DANMAKU_URL_PATTERN2 = Pattern.compile(
            "(?i)(?:https?://)?[^ ]*?/comment/([0-9]+)\\b"
    );
    // 用于apiBase前缀之后的相对匹配
    private static final Pattern DANMAKU_EPID_AFTER_BASE = Pattern.compile(
            "(?i)/api/v2/comment/([0-9]+)\\b"
    );
    // 完整弹幕URL（含query串），用于全文相等判断
    private static final Pattern DANMAKU_FULL_URL_PATTERN = Pattern.compile(
            "(?i)[^ \\t]*?/api/v2/comment/[0-9]+[^ \\t]*"
    );
    // 通用完整格式：.../comment/{epId}...
    private static final Pattern DANMAKU_FULL_URL_PATTERN2 = Pattern.compile(
            "(?i)(?:https?://)?[^ \\t]*?/comment/[0-9]+[^ \\t]*"
    );
                ...
                synchronized (runLock) {
                    try {
                        ...
                        if (act != null && !act.isFinishing()) {
                            // 重标记播放器弹幕菜单项（用剧集名替换URL显示），任意界面都扫描
                            mainHandler.post(new Runnable() {
                                @Override
                                public void run() {
                                    try {
                                        relabelDanmakuMenu(act);
                                    } catch (Exception e) {
                                        DanmakuSpider.log("❌ 弹幕菜单重标记异常: " + e.getMessage());
                                    }
                                }
                            });
...
//            DanmakuSpider.log("✅ 检测到同一个视频，忽略重复触发");
    ...
    public static String extractEpisodeNum(String title) {
        ...
        if (!candidates.isEmpty()) {
            ...
            return checkForSpecialPrefix(processedTitle, best.number, best.start);
        }
        ...
        if (!possibleEpisodes.isEmpty()) {
            ...
            for (String num : possibleEpisodes) {
                ...
                if (value >= 1 && value <= 99) {
                    int pos = processedTitle.indexOf(num);
                    return pos >= 0 ? checkForSpecialPrefix(processedTitle, num, pos) : num;
                }
            }
            // 如果没有1-99的，返回第一个
            String first = possibleEpisodes.get(0);
            int pos = processedTitle.indexOf(first);
            return pos >= 0 ? checkForSpecialPrefix(processedTitle, first, pos) : first;
        }
    ...
    private static boolean isLikelyEpisodeNumber(String title, String number, int start, int end) {
        ...
        return hasEpisodeKeyword || (!hasNonEpisodeKeyword && numValue >= 1 && numValue <= 999);
    }
    ...
    private static String checkForSpecialPrefix(String processedTitle, String number, int startPos) {
        if (TextUtils.isEmpty(processedTitle) || TextUtils.isEmpty(number) || startPos <= 0) {
            return number;
        }
        String before = processedTitle.substring(0, startPos).trim();
        String[] keywords = {"彩蛋", "预告", "采访", "特典", "花絮", "幕后"};
    ...
    // 重标记播放器弹幕菜单项：将弹幕源URL文本替换为"剧名 集标题"
    private static void relabelDanmakuMenu(Activity activity) {
        if (activity == null || activity.isFinishing()) return;
        java.util.List<View> collected = new java.util.ArrayList<>();
        String routes = "";

        // 路A: WindowManagerGlobal.getWindowViews()
        try {
            Class<?> wmgCls = Class.forName("android.view.WindowManagerGlobal");
            Object wmg = wmgCls.getMethod("getInstance").invoke(null);
            java.util.List<View> views = (java.util.List<View>) wmgCls.getMethod("getWindowViews").invoke(wmg);
            if (views != null && !views.isEmpty()) {
                collected.addAll(views);
                routes += "A";
            }
        } catch (Exception e) {
            DanmakuSpider.log("[弹幕菜单扫描] 路A失败: " + e);
        }

        // 路B: 反射 WindowManagerGlobal.mViews 字段
        try {
            Class<?> wmgCls = Class.forName("android.view.WindowManagerGlobal");
            Object wmg = wmgCls.getMethod("getInstance").invoke(null);
            java.lang.reflect.Field f = wmgCls.getDeclaredField("mViews");
            f.setAccessible(true);
            java.util.List<View> views = (java.util.List<View>) f.get(wmg);
            if (views != null && !views.isEmpty()) {
                collected.addAll(views);
                routes += "B";
            }
        } catch (Exception e) {
            DanmakuSpider.log("[弹幕菜单扫描] 路B失败: " + e);
        }

        // 路C: WindowManagerImpl.getDefault().mGlobal.mViews
        try {
            Class<?> wmCls = Class.forName("android.view.WindowManagerImpl");
            Object wm = wmCls.getMethod("getDefault").invoke(null);
            java.lang.reflect.Field globalF = wmCls.getDeclaredField("mGlobal");
            globalF.setAccessible(true);
            Object global = globalF.get(wm);
            java.lang.reflect.Field viewsF = global.getClass().getDeclaredField("mViews");
            viewsF.setAccessible(true);
            java.util.List<View> views = (java.util.List<View>) viewsF.get(global);
            if (views != null && !views.isEmpty()) {
                collected.addAll(views);
                routes += "C";
            }
        } catch (Exception e) {
            DanmakuSpider.log("[弹幕菜单扫描] 路C失败: " + e);
        }

        // 路D: 反射遍历顶层Activity的DialogFragment弹窗（FongMi系 DanmakuDialog 是 BottomSheetDialogFragment）
        try {
            java.lang.reflect.Method getSfm;
            try {
                getSfm = activity.getClass().getMethod("getSupportFragmentManager");
            ...
            if (getSfm != null) {
                ...
                if (fm != null) {
                    java.lang.reflect.Method getFragments = fm.getClass().getMethod("getFragments");
                    Object fragList = getFragments.invoke(fm);
                    if (fragList instanceof java.util.List) {
                        for (Object f : (java.util.List<?>) fragList) {
                            if (f == null) continue;
                            java.lang.reflect.Method getDialog;
                            try {
                                getDialog = f.getClass().getMethod("getDialog");
                            } catch (NoSuchMethodException nsme2) {
                                getDialog = (f.getClass().getSuperclass() != null) ? f.getClass().getSuperclass().getMethod("getDialog") : null;
                            }
                            if (getDialog == null) continue;
                            Object dialog = getDialog.invoke(f);
            ...
            DanmakuSpider.log("[弹幕菜单扫描] 路D失败: " + e);
        }
        ...
        // 按身份去重（IdentityHashMap）
        java.util.Map<View, Boolean> uniq = new java.util.IdentityHashMap<>();
        for (View v : collected) if (v != null) uniq.put(v, Boolean.TRUE);
        java.util.List<View> windowViews = new java.util.ArrayList<>(uniq.keySet());

        DanmakuSpider.log("🔍 [弹幕菜单扫描] 窗口数: " + windowViews.size() + " (路由: " + routes + ")");
        for (View root : windowViews) {
            DanmakuSpider.log("[弹幕菜单扫描] 窗口类: " + (root.getClass().getName()) + " 子数: " + (root instanceof ViewGroup ? ((ViewGroup) root).getChildCount() : 0));
            relabelInView(root);
        }
    }

    // 深度遍历视图树，替换匹配弹幕URL文本的TextView
    private static void relabelInView(View view) {
        if (view == null) return;
        if (view instanceof TextView) {
            TextView tv = (TextView) view;
            ...
            String epIdStr = matchDanmakuEpId(text);
            if (epIdStr == null) return;
            // 关键保护：仅当TextView全文就是该弹幕URL时才替换，
            // 避免误改日志查看器等包含URL的长文本（如整段日志）。
            String url = matchDanmakuUrl(text);
            if (url == null || !text.trim().equalsIgnoreCase(url)) return;
            String label = buildDanmakuLabel(epIdStr);
            if (label == null) {
                DanmakuSpider.log("[弹幕菜单扫描] 匹配到URL但无法生成标签, epId=" + epIdStr + " 文本: " + text);
                return;
            }
            if (!label.equals(text)) {
                tv.setText(label);
                tv.setTag("danmaku_labeled");
                DanmakuSpider.log("📺 弹幕菜单项: " + text + " → " + label);
            }
        } else if (view instanceof ViewGroup) {
            ViewGroup group = (ViewGroup) view;
            int count = group.getChildCount();
            for (int i = 0; i < count; i++) {
                relabelInView(group.getChildAt(i));
            }
        }
    }

    // 从文本中提取弹幕epId（多重匹配）
    private static String matchDanmakuEpId(String text) {
        try {
            Matcher m = DANMAKU_URL_PATTERN.matcher(text);
            if (m.find()) return m.group(1);
            m = DANMAKU_URL_PATTERN2.matcher(text);
            if (m.find()) return m.group(1);
            // 用当前所有已知apiBase前缀匹配
            for (DanmakuItem item : DanmakuManager.lastDanmakuItemMap.values()) {
                if (item == null || TextUtils.isEmpty(item.getApiBase())) continue;
                String prefix = item.getApiBase();
                int idx = text.indexOf(prefix);
                if (idx < 0) continue;
                String rest = text.substring(idx + prefix.length());
                Matcher m2 = DANMAKU_EPID_AFTER_BASE.matcher(rest);
    ...
    // 提取文本中匹配到的完整弹幕URL（含query），用于全文相等判断（防止误改长文本）
    private static String matchDanmakuUrl(String text) {
        try {
            Matcher m = DANMAKU_FULL_URL_PATTERN.matcher(text);
            if (m.find()) return m.group(0).trim();
            m = DANMAKU_FULL_URL_PATTERN2.matcher(text);
            if (m.find()) return m.group(0).trim();
        } catch (Exception e) {
            // 忽略异常
        }
    ...
    // 根据epId生成友好的剧集标签
    private static String buildDanmakuLabel(String epIdStr) {
        try {
            int epId = Integer.parseInt(epIdStr);
            DanmakuItem item = DanmakuManager.lastDanmakuItemMap.get(epId);
            if (item != null) {
                String anime = !TextUtils.isEmpty(item.getAnimeTitle()) ? item.getAnimeTitle()
                        : (!TextUtils.isEmpty(item.getTitle())
                        ? (item.getTitle().contains("from")
                        ? item.getTitle().substring(0, item.getTitle().indexOf("from")).trim()
                        : item.getTitle().trim())
                        : "");
                String ep = !TextUtils.isEmpty(item.getEpTitle()) ? item.getEpTitle() : ("第" + epId + "集");
                return TextUtils.isEmpty(anime) ? ep : anime + " " + ep;
            }
            EpisodeInfo info = lastEpisodeInfo;
            if (info != null) {
                StringBuilder sb = new StringBuilder();
                if (!TextUtils.isEmpty(info.getSeriesName())) sb.append(info.getSeriesName());
                String season = info.getEpisodeSeasonNum();
                String ep = info.getEpisodeNum();
                if (!TextUtils.isEmpty(season) && !TextUtils.isEmpty(ep)) {
                    if (sb.length() > 0) sb.append(" ");
                    sb.append("S").append(season).append("E").append(ep);
                } else if (!TextUtils.isEmpty(ep)) {
                    if (sb.length() > 0) sb.append(" ");
                    sb.append("第").append(ep).append("集");
                }
...
//                    DanmakuSpider.log("[按钮注入] 按钮已存在，跳过");
                ...
                btn.setOnLongClickListener(new View.OnLongClickListener() {
                    ...
                    public boolean onLongClick(View v) {
                        if (parent.getContext() instanceof Activity) {
                            ...
                            DanmakuSpider.log("[按钮长按] 打开远程搜索二维码");
     ...
     * @param activity
     */
    public static void showLeoButtonMenu(final Activity activity) {
        try {
            android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(activity);
            builder.setTitle("弹幕设置");
