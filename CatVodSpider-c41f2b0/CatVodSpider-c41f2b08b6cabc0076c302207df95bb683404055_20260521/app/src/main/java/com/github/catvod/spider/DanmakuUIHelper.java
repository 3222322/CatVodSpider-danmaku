                ...
                if (animeItems.size() == 1) {
                    ...
                    groupBtn.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            Object[] currentStateInfo = (Object[]) groupBtn.getTag();
                            boolean isExpanded = (int) currentStateInfo[0] == 1;
                            ...
                            if (isExpanded) {
                                ...
                                for (DanmakuItem it : animeItems) {
                                    String epText = it.getEpTitle() != null ? it.getEpTitle() : "";
                                    float tw = paint.measureText(epText);
                                    if (tw > maxTextWidth) maxTextWidth = tw;
                                }
                                int btnWidthPx = (int) Math.ceil(maxTextWidth) + dpToPx(activity, 36);
                                int containerWidth = resultContainer.getWidth();
                                int columns = Math.max(1, containerWidth / Math.max(btnWidthPx, 1));

                                GridLayout gridLayout = new GridLayout(activity);
                                gridLayout.setColumnCount(columns);
                                gridLayout.setRowCount(GridLayout.UNDEFINED);
                                gridLayout.setUseDefaultMargins(false);
                                gridLayout.setPadding(0, dpToPx(activity, 4), 0, dpToPx(activity, 4));

                                for (DanmakuItem it : animeItems) {
                                    Button subItem = createResultButton(activity, it, dialog, isTemplate3);
                                    subItem.setPadding(dpToPx(activity, 14), dpToPx(activity, 10), dpToPx(activity, 14), dpToPx(activity, 10));
                                    ...
                                    int m = dpToPx(activity, 4);
                                    gp.setMargins(m, m, m, m);
                                    subItem.setLayoutParams(gp);
                                    gridLayout.addView(subItem);
                                }
                            ...
                            resultContainer.post(() -> {
                                ...
                                for (int i = 0; i < resultContainer.getChildCount(); i++) {
                                    ...
                                    if (child instanceof Button && child.getTag() instanceof DanmakuItem) {
                                        ...
                                        GridLayout gl = (GridLayout) child;
                                        for (int j = 0; j < gl.getChildCount(); j++) {
                                            View gc = gl.getChildAt(j);
                                            if (gc instanceof Button && gc.getTag() instanceof DanmakuItem) {
                                                DanmakuItem item = (DanmakuItem) gc.getTag();
                                                if (item.getDanmakuUrl() != null && item.getDanmakuUrl().equals(DanmakuManager.lastDanmakuUrl)) {
                                                    targetView = gc;
                                                    break;
                                                }
