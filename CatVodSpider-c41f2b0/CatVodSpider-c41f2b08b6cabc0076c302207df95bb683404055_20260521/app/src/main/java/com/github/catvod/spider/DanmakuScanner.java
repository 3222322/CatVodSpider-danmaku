...
//                if (text.equals("硬解") || text.equals("软解") || text.equals("字幕") || text.equals("视轨") || text.equals("音轨")) {
    ...
    private static void injectButton(ViewGroup parent, TextView anchor) {
        try {
            ...
            if (existing != null) {
                ...
                if (isTargetAnchor) {
                    ...
                    if (anchorIndex >= 0) {
                        ...
                        parent.addView(existing, anchorIndex + 1);
                    }
                }
                return;
            }

            // 在非RecyclerView的ViewGroup中注入按钮
            if (isInRecyclerView(parent)) {
                DanmakuSpider.log("[按钮注入] 跳过RecyclerView中的注入");
                return;
            }

            // 创建Leo弹幕按钮
            final TextView danmuBtn = new TextView(anchor.getContext());
            danmuBtn.setTag("danmu_button");
            danmuBtn.setText("弹幕搜索");
            danmuBtn.setTextSize(12);
            danmuBtn.setTextColor(Color.WHITE);
            danmuBtn.setPadding(8, 4, 8, 4);
            danmuBtn.setGravity(Gravity.CENTER);
            danmuBtn.setSingleLine(true);
            danmuBtn.setBackgroundColor(0xFFFF4444);
            danmuBtn.setVisibility(View.VISIBLE);
            danmuBtn.setClickable(true);
            danmuBtn.setFocusable(true);
            danmuBtn.setFocusableInTouchMode(true);

            // 设置layout参数
            int btnHeight = anchor.getMeasuredHeight() > 0 ? anchor.getMeasuredHeight() : ViewGroup.LayoutParams.WRAP_CONTENT;
            ViewGroup.LayoutParams params;

            if (parent instanceof LinearLayout) {
                params = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, btnHeight);
            } else if (parent instanceof android.widget.RelativeLayout) {
                params = new android.widget.RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, btnHeight);
            } else {
                params = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, btnHeight);
            }
            ...
            // 确定插入位置
            int insertIndex = parent.indexOfChild(anchor) + 1;
            if (insertIndex <= 0) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                danmuBtn.setElevation(4);
                danmuBtn.setTranslationZ(2);
            }

            parent.addView(danmuBtn, insertIndex, params);
            isLeoButtonInjected = true;

            DanmakuSpider.log("[按钮注入] ✅ Leo弹幕按钮注入成功");

            // 设置点击事件（保持不变）
            danmuBtn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    handleDanmakuButtonClick(v);
                }
            });

            // 长按分享弹幕搜索页面二维码
            danmuBtn.setOnLongClickListener(new View.OnLongClickListener() {
                @Override
                public boolean onLongClick(View v) {
                    handleDanmakuButtonLongClick(v);
                    return true;
                }
            });

        } catch (Exception e) {
            DanmakuSpider.log("[按钮注入] 注入失败: " + e.getMessage());
            e.printStackTrace();
        }
    }

    // 检查是否在RecyclerView中
    private static boolean isInRecyclerView(ViewGroup parent) {
    ...
    // 处理弹幕按钮点击
    private static void handleDanmakuButtonClick(View v) {
        DanmakuSpider.log("[按钮点击] 打开搜索对话框");
        Activity act = (Activity) v.getContext();
        // 显示搜索对话框
        DanmakuUIHelper.showSearchDialog(act, lastEpisodeInfo);
    }

    // 处理弹幕按钮长按（分享二维码等）
    private static void handleDanmakuButtonLongClick(View v) {
        Activity act = (Activity) v.getContext();
        DanmakuSpider.log("[按钮长按] 显示Web搜索页面");

        // 获取IP地址
        String ip = NetworkUtils.getLocalIpAddress();
        if (ip != null) {
            String webUrl = "http://" + ip + ":9810";
            Utils.safeShowToast(act, "Web搜索地址: " + webUrl + "\n长按复制链接");
        } else {
            Utils.safeShowToast(act, "无法获取IP地址");
        }
    }

    private static void startAutoSearch(EpisodeInfo episodeInfo, Activity activity) {
    ...
    private static void iterativeAutoSearch(EpisodeInfo episodeInfo, Activity activity) {
        if (episodeInfo == null) return;

        List<String> episodeNames = episodeInfo.getEpisodeNames();
        if (episodeNames == null || episodeNames.isEmpty()) {
            DanmakuSpider.log("⚠️ 没有可用的剧集名进行搜索");
            return;
        }

        String episodeNum = episodeInfo.getEpisodeNum();

        DanmakuSpider.log("🔍 开始自动搜索: episodeName=" + episodeNames.toString() + ", episodeNum=" + episodeNum);

        for (String name : episodeNames) {
            if (TextUtils.isEmpty(name)) continue;

            if (!DanmakuScanner.isMonitoring) {
                break;
            }

            // 创建搜索信息
            EpisodeInfo searchInfo = new EpisodeInfo();
            searchInfo.setEpisodeNames(episodeNames);
            searchInfo.setEpisodeNum(episodeNum);
            searchInfo.setEpisodeYear(episodeInfo.getEpisodeYear());
            searchInfo.setEpisodeSeasonNum(episodeInfo.getEpisodeSeasonNum());

            long searchStartTime = System.currentTimeMillis();
            LeoDanmakuService.SearchResult currentResult = LeoDanmakuService.autoSearch(name, searchInfo, activity);
            long searchTime = System.currentTimeMillis() - searchStartTime;

            if (currentResult.found && currentResult.similarity >= 1.0) {
                // 精确匹配，直接推送
                DanmakuSpider.log("🎯 精确匹配，episodeNames=" + episodeNames.toString() + ", episodeNum=" + episodeNum);

                scheduleDelayedPush(currentResult.item, activity, name, episodeNum != null ? episodeNum : name);
                break;
            } else if (currentResult.found && currentResult.similarity >= 0.85) {
                // 高相似度匹配
                DanmakuSpider.log("📊 高相似度匹配: " + currentResult.similarity + "，episodeNames=" + episodeNames.toString() + ", episodeNum=" + episodeNum);

                scheduleDelayedPush(currentResult.item, activity, name, episodeNum != null ? episodeNum : name);
                break;
            } else if (!currentResult.found && !episodeNames.isEmpty() && activity != null && !activity.isFinishing()) {
                // 没有找到任何匹配
                // 如果是自动搜索且没有获取到结果就继续尝试下一个api
                DanmakuSpider.log("❌ 搜索 '" + name + "' 没有找到匹配，当前结果: " + (currentResult.item != null ? currentResult.item.title : "null"));
            }
        }
    ...
    public static boolean hasPendingPushes() {
        return !pendingPushes.isEmpty();
    }

    // 获取待推送任务数量
    public static int getPendingPushCount() {
        return pendingPushes.size();
    }
}
